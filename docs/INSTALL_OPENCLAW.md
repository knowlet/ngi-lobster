...
Queue entries now keep a runtime-owned `content_kind` such as `article` or `video_transcript`, and the linked-content evidence/markdown artifacts preserve that same intent for auditability.
