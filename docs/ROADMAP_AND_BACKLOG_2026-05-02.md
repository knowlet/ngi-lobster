...
4. **每日/每次 run 的可讀摘要體系**
   ...
   - 2026-05-05 06:03+08:00：已加固 ops-health latest NGI active-target identity projection；`market_target.market_id`、`market_target.market_name`、`target_detail.market_id`、`target_detail.market_question` 與 `probability_mode` 會以 canonical non-empty string 投影，避免 operator-facing target / reselection evidence 帶前後空白。
...
- ops-health freshness timestamp helper 也必須維持同一 UTC-offset 契約；直接計算 freshness 時，timezone-less datetime 不能被隱性當成 UTC。
- ops-health freshness timestamp helper 的 reference time 也必須維持 timezone-aware `datetime` 契約；測試或內部呼叫不能用 naive 或 malformed `now` 洩漏底層 Python 型別錯誤。
...
- ops-health 的 `latest_ngi_timestamp_utc` output 必須 canonicalize 成 UTC offset；即使 input timestamp 帶非 UTC offset，operator-facing `_utc` 欄位也不能保留原始 offset 字串。
