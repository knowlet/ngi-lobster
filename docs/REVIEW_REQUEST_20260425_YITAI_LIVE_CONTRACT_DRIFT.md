...
3. Where is the exact live writer/runtime seam that currently strips or never populates those four fields on the real artifact path, and where should the downstream-facing reason-code mapping happen?
...
- the downstream-facing `reason_code` is explicitly on-contract and not the internal `no_novelty_within_24h` label,
