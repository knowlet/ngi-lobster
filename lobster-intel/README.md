...
The source-fusion CLI now also reads `lobster-intel/data/runtime/sources/firehose-tracker/latest.json` by default, with every non-absolute input/output path resolved from the requested `--workspace`, so fusion artifacts report how many Firehose events were analyzed, which normalized Firehose `run_id` supplied that summary, and the latest event and collection timestamps. Absolute paths (including `~`-expanded home paths) bypass workspace anchoring. If that Firehose artifact does not exist yet, source fusion now keeps running and emits zero-count Firehose audit metadata instead of failing the whole CLI. Firehose ranking/filtering still remains a separate unfinished slice.
