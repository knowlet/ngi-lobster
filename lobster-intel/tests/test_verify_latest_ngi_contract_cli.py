...
def test_verify_latest_ngi_contract_cli_accepts_on_contract_payload(tmp_path: Path):
    ...
    payload = {
        ...
        "gap_triggered": True,
        "divergence_pp": 19.0,
        "first_principles_minus_market_pp": -19.0,
...
def test_verify_latest_ngi_contract_cli_fails_off_contract_reason_code(tmp_path: Path):
    ...
    payload = {
        ...
        "gap_triggered": True,
        "divergence_pp": 22.5,
        "first_principles_minus_market_pp": 22.5,
        "direction": "first_principles_above_market",
        "alert_disposition": {
            "should_send": False,
            "decision": "suppressed",
...
def test_verify_latest_ngi_contract_cli_fails_when_gap_triggered_missing_signed_divergence_fields(tmp_path: Path):
    repo = Path(__file__).resolve().parents[2]
    payload = {
        "market_target": {
            "market_id": "1517836",
        ...
        "first_principles_probability": 0.1156,
        "gap_triggered": True,
        "alert_disposition": {
            "should_send": False,
            "decision": "suppressed",
    ...
    output = json.loads(result.stdout)
    assert output["status"] == "contract_violation"
    assert "missing_signed_divergence_field:divergence_pp" in output["issues"]
    assert "missing_signed_divergence_field:first_principles_minus_market_pp" in output["issues"]
    assert "missing_signed_divergence_field:direction" in output["issues"]
