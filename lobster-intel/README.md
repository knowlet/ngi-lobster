...
The source-fusion CLI now also reads `lobster-intel/data/runtime/sources/firehose-tracker/latest.json` by default, so fusion artifacts report how many Firehose events were analyzed plus the latest event and collection timestamps. Firehose ranking/filtering still remains a separate unfinished slice.
